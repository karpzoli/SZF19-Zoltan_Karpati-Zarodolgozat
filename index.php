...
<section class="LogonAndMainPage">
...
if(isset($_COOKIE['ha_dist_id'])){
	...
	//$tomb=mysql_fetch_array($userInfo);
	//extract($tomb);
	}

// LOGIN
if(isset($_POST['loginButton'])){ 
	require_once("./pages/connect.php"); 			//within we have the DB connnection function
	db_connect();
	extract($_POST); 						//imports variables into the local symbol table from an array
	//$pw=md5($pw);						//calculates the MD5 hash of a string
	$searchInDb="SELECT * FROM users WHERE uid='$user' AND password='$pw'"; //AND role > 0 => deactivated user
	$userInfo=mysql_query($searchInDb) or die(mysql_error()); //erforms a query against the database.
	$status=mysql_num_rows($userInfo); 				//returns the number of rows in a result set.
...
echo '<div class="col-md-4 multi-horizontal">';
echo '<div class="form-group">';
echo '<input type="text" class="form-control" name="user" placeholder="username" required>';
echo '</div>';
echo '</div>';
echo '<div class="col-md-4 multi-horizontal">';
echo '<div class="form-group">';
...
echo '<button type="submit" class="btn btn-primary btn-form display-4" name="loginButton">LOGIN</button>';
echo '</span>';
echo '</form>';
} 
